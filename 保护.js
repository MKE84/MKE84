...
const Utils = {
  ...
  createServiceGroups(config, regionGroupNames, ruleProviders, rules) {
    ...
    Config.services.forEach(service => {
      ...
      const proxies = service.proxies || ["国家节点", ...(service.proxiesOrder || []), ...regionGroupNames, "直连模式"];
...
class CentralManager extends EventEmitter {
  ...
  async smartDispatchNode(user, nodes, context) {
    ...
    try { if (this.lruCache && bestNode.id) this.lruCache.set(cacheKey, bestNode.id); } catch (e) { Logger.debug("缓存国家节点选择结果失败:", e.message); }
    return bestNode;
  }
  ...
  processConfiguration(config) {
    ...
    try {
      ...
      if (otherProxyGroups.length > 0) regionGroupNames.push("文字节点");
    ...
    try {
      safeConfig["proxy-groups"] = [{
        ...(Config.common?.proxyGroup || {}), name: "国家节点", type: "select",
        proxies: [...regionGroupNames, "直连模式"], icon: ICONS.Proxy
      }];
    ...
    try {
      ...
      if (!safeConfig.proxies.some(p => p && p.name === "直连模式")) { safeConfig.proxies.push({ name: "直连模式", type: "direct" }); }
    } catch (e) { Logger.warn("添加直连模式代理失败:", e.message); }
    ...
    try {
      if (otherProxyGroups.length > 0) {
        safeConfig["proxy-groups"].push({
          ...(Config.common?.proxyGroup || {}),
          name: "文字节点", type: "select", proxies: otherProxyGroups,
          icon: ICONS.WorldMap
        });
      }
    } catch (e) { Logger.warn("添加文字节点组失败:", e.message); }
...
const Config = {
  ...
  ruleOptions: {
    apple: false, microsoft: true, github:false, google: true, openai: false, spotify: false,
    youtube: false, bahamut: false, netflix: false, tiktok: false, disney: false, pixiv: false,
    hbo: false, biliintl: false, tvb: false, hulu: false, primevideo: false, telegram: true,
    line: false, whatsapp: false, games: true, japan: false, tracker: true, ads: false
  },
  preRules: [
    "RULE-SET,applications,下载服务",
  ...
  services: [
    ...
    { id: "biliintl", rule: ["GEOSITE,biliintl,哔哩哔哩东南亚"], name: "哔哩哔哩东南亚", url: "https://www.bilibili.tv/", icon: ICONS.Bilibili3, proxiesOrder: ["国家节点", "直连模式"] },
    { id: "bahamut", rule: ["GEOSITE,bahamut,巴哈姆特"], name: "巴哈姆特", url: "https://ani.gamer.com.tw/ajax/getdeviceid.php", icon: ICONS.Bahamut, proxiesOrder: ["国家节点", "直连模式"] },
    ...
    { id: "tracker", rule: ["GEOSITE,tracker,跟踪保护"], name: "跟踪保护", icon: ICONS.Reject, proxies: ["REJECT", "直连模式", "国家节点"] },
    { id: "ads", rule: ["GEOSITE,category-ads-all,广告拦截", "RULE-SET,adblockmihomo,广告拦截"], name: "广告拦截", icon: ICONS.Advertising, proxies: ["REJECT", "直连模式", "国家节点"], ruleProvider: {name: "adblockmihomo", url: URLS.rulesets.adblock_mihomo_mrs, format: "mrs", behavior: "domain"} },
  ...
  common: {
    ...
    defaultProxyGroups: [
      { name: "下载服务", icon: ICONS.Download, proxies: ["直连模式", "REJECT", "国家节点", "国内网站"] },
      { name: "外网网站", icon: ICONS.StreamingNotCN, proxies: ["国家节点", "国内网站"] },
      { name: "国内网站", url: "http://wifi.vivo.com.cn/generate_204", icon: ICONS.StreamingCN, proxies: ["直连模式", "国家节点"] }
    ],
    postRules: ["GEOSITE,private,DIRECT", "GEOIP,private,DIRECT,no-resolve", "GEOSITE,cn,国内网站", "GEOIP,cn,国内网站,no-resolve", "MATCH,外网网站"]
  }
};
